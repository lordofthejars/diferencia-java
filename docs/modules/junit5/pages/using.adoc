...
@DiferenciaCore(primary = "http://now.httpbin.org", candidate = "http://now.httpbin.org", // <2>
    config = @DiferenciaConfig(noiseDetection = true, secondary = "http://now.httpbin.org")) // <3>
public class DiferenciaExtensionTest {

    @Test
    public void should_use_diferencia_to_detect_any_possible_regression(Diferencia diferencia) // <4>
            throws IOException {
